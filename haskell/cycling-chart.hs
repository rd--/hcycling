...
to_diff f i =
    case i of
      (x:xs) -> map (zipWith f x) (x:xs)
      _ -> undefined

gradient_v_chart :: Maybe FilePath -> Maybe (R->R->R) ->
                    (R,R) -> [R] -> [R] -> IO ()
...
let c f = gradient_v_chart Nothing f (0.05,250) [65,70..100] [-5,-4.5..20]
c Nothing
c (Just (-))
c (Just (/))
